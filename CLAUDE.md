...
`.claude/settings.json` で `Read`/`Write`/`Edit`/`Glob`/`Grep` を自動許可済み。
`Bash`（ビルド・テスト実行）は自動許可されていないため、エージェントが `swift build` 等を実行する際はユーザー確認が必要。

*Rationale: Bash は破壊的コマンドを含むため自動許可しない。ファイル操作系ツールは実装エージェントが頻繁に使うため自動許可してコンテキスト中断を防ぐ。*
